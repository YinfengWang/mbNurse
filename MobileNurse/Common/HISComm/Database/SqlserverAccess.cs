...
using DBAdapter = System.Data.SqlClient.SqlDataAdapter;
using DBCommand = System.Data.SqlClient.SqlCommand;
using DBCmdBuilder = System.Data.SqlClient.SqlCommandBuilder;
...
namespace HISPlus
{
	...
	public class SqlserverAccess: DbAccess
    {
        ...
        public SqlserverAccess(string connectString)
		{
        ...
        public override int Update(ref DataSet dsChanged)
        {
            ...
            foreach(DataRow dr in dsChanged.Tables[0].Rows)
            {
                ...
                for(int i = 0; i < dcPrimary.Length; i++)
                {
                    ...
                    {
                        filter += SQL.GetSqlDbDate((DateTime)(dr[dc.ColumnName]));
                    }
                    else
                    {
                ...
                if (dr.RowState == DataRowState.Added)
                {
                    ...
                    foreach(DataColumn dc1 in dsChanged.Tables[0].Columns)
                    {
                        ...
                        if (dc1.DataType == dateTime)
                        {
                            ...
                            {
                                valList += SQL.GetSqlDbDate((DateTime)(dr[dc1.ColumnName]));
                            }
                        }
                        else
                        {
                ...
                if (dr.RowState == DataRowState.Modified)
                {
                    ...
                    foreach(DataColumn dc2 in dsChanged.Tables[0].Columns)
                    {
                        ...
                        if (dc2.DataType == dateTime)
                        {
                            ...
                            {
                                valPair += SQL.GetSqlDbDate((DateTime)(dr[dc2.ColumnName]));
                            }
                        }
                        else
                        {
