	...
	public class BodyTemper
	{
		...
		public void DrawOtherNursingEventText(ref Graphics grfx, Point ptLeftRight, Size szRng)
		{
			...
			for(int i = 0; i < drFind.Length; i++)
			{
                ...
                // 其它护理事件的名称 + 时间
                if (strValue.IndexOf("入院") >= 0 
				    || strValue.IndexOf("死亡") >= 0
				    || strValue.IndexOf("心跳停止") >= 0)
		        {			        
		        ...
		        {			        
				    strValue = "转 " + drFind[i]["VITAL_SIGNS_CVALUES"].ToString();
				    grfx.DrawString(strValue, Font, brushRed, rect, this.drawFormat);
				}
                
				// 其它护理事件的名称
                if (strValue.IndexOf("分娩") >= 0 
				    || strValue.IndexOf("出院") >= 0
				    || strValue.IndexOf("手术") >= 0)
		        {
                    if (strValue.Trim().Equals("手术") == true)
                    {
                        strValue = "手  术";
                    }
