...
namespace HISPlus
{
    public class UserDbI
    {
        ...
        {
            userName = userName.ToUpper().Trim();
            string strSql = @"SELECT M.DEPT_CODE,N.DEPT_NAME FROM (      " +
                              "  SELECT C.DEPT_CODE      " +
                              "  FROM STAFF_DICT  A ,DEPT_DICT B ,DEPT_VS_GROUP C      " +
                              "  WHERE A.DEPT_CODE = B.DEPT_CODE      " +
                              "  AND B.GROUP_CODE = C.GROUP_CODE      " +
                              "  AND A.EMP_NO='" + userName + "')M,DEPT_DICT N      " +
                              "  WHERE M.DEPT_CODE = N.DEPT_CODE";
            return _oraConnect.SelectData_NoKey(strSql);
        }
        ...
        /// <returns></returns>
        public DataSet GetUserDeptInfo(string userName)
        {
            userName = userName.ToUpper().Trim();
            string sql = "SELECT B.DEPT_CODE,B.DEPT_NAME FROM STAFF_DICT A,DEPT_DICT B WHERE A.DEPT_CODE = B.DEPT_CODE AND USER_NAME = " + SQL.SqlConvert(userName);
            return _oraConnect.SelectData(sql);
        }
        /// <summary>
        /// 保存用户信息
        ...
        public DataSet GetUserRights(string appCode, string userId)
        {
            ...
            sb.Append("AND APP_DICT.APP_CODE = MENU_MANAGER.APP_CODE ");
            sb.Append("AND ROLE_RIGHTS.MODULE_CODE = MENU_MANAGER.MODULE_CODE ");
