...
namespace TemperatureBLL
{
    public class BodyTemperatureCom
    {
        ...
        /// <returns>TRUE: 成功; FALSE: 失败</returns>
        public bool GetVitalSignsRecDate(string patientId, int visitId, ref DateTime minDate, ref DateTime maxDate)
        {
            // 获取护理事件
            string sql = string.Empty;

            sql = "SELECT MIN(VITAL_SIGNS_REC.TIME_POINT)as MINDATE,MAX(VITAL_SIGNS_REC.TIME_POINT) as MAXDATE ";
            sql += "FROM VITAL_SIGNS_REC, NURSING_ITEM_DICT ";
            sql += "WHERE VITAL_SIGNS_REC.PATIENT_ID = " + SQL.SqlConvert(patientId);
            sql += "AND VITAL_SIGNS_REC.VISIT_ID = " + SQL.SqlConvert(visitId.ToString());
            sql += "AND VITAL_SIGNS_REC.WARD_CODE = NURSING_ITEM_DICT.WARD_CODE ";
            sql += "AND VITAL_SIGNS_REC.VITAL_CODE = NURSING_ITEM_DICT.VITAL_CODE ";
        ...
        {
            string sql = "SELECT * FROM APP_CONFIG WHERE APP_CODE ='001' ";
