...
namespace Dicom.Imaging.Codec
{
    public class DicomTranscoder
    {
        ...
        public static IDicomCodec GetCodec(DicomTransferSyntax syntax)
        {
            ...
            if (_codecs.Count == 0)
            {
                LoadCodecs(AppDomain.CurrentDomain.BaseDirectory, "Dicom.Native*.dll");
                if (_codecs.Count == 0)
                {
                    string str = string.Empty;
                    str += "Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location): "
                        + Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) + Environment.NewLine;
                    str += "AppDomain.CurrentDomain.BaseDirectory: " + AppDomain.CurrentDomain.BaseDirectory + Environment.NewLine;
                    throw new Exception(
                       str + " Not Find Dicom.Native*.dll ");
                }
            }
