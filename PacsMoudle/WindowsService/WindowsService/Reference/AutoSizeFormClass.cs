...
namespace PACSMonitor
{
    public class AutoSizeFormClass
    {
        ...
        private void AutoScaleControl(Control ctl, float wScale, float hScale)
        {
            ...
            {
                //获得控件原有的位置和大小信息
                ctrLeft0 = oldCtrl[Math.Min(ctrlNo,oldCtrl.Count-1)].Left;
                ctrTop0 = oldCtrl[Math.Min(ctrlNo, oldCtrl.Count - 1)].Top;
                ctrWidth0 = oldCtrl[Math.Min(ctrlNo, oldCtrl.Count - 1)].Width;
                ctrHeight0 = oldCtrl[Math.Min(ctrlNo, oldCtrl.Count - 1)].Height;
                ctrFontSize0 = oldCtrl[Math.Min(ctrlNo, oldCtrl.Count - 1)].FontSize;
                //设置控件新的位置和大小信息。
