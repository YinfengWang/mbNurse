	...
	public class LoginCom
	{
        ...
        public bool LoadAppSetting_Local()
        {
            try
            {
                ...
                string oraConnectHis = enDecrypt.Decrypt(ConfigurationManager.AppSettings["StrConnHis"].ToString());
                string oraConnectMobile = enDecrypt.Decrypt(ConfigurationManager.AppSettings["StrConnMobile"].ToString());
                ...
                string pwd = ConnectionStringHelper.GetParameter(oraConnectHis, "password");
                GVars.OracleHis.ConnectionString = ConnectionStringHelper.GetOleConnectionString(dataSrc, user, pwd);
            }
            catch (Exception e)
            {
                throw e;
