...
namespace HISPlus
{
    public class ServerCreateSyncInfo
    {
            ...
            while(_blnExit == false)
            {
                try
                {
                    ...
                    for(int i = 0; i < syncInfos.Count; i++)
                    {
                        ...
                        CreateSyncInfo(getSqlContent(syncInfo.SrcSqlFile), syncInfo.DestSql, syncInfo.TableName);
        ...
        private DataSet CreateSyncInfo(string sqlSrc, string sqlDest, string tableNameDest)
        {
            // 获取数据
