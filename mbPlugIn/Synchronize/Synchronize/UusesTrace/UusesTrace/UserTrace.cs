...
namespace HISPlus
{
    public partial class UserTrace : FormDo
    {

        #region 变量
        private static ServerCreateSyncInfo tracer = new ServerCreateSyncInfo();
        private static Thread threadTracer;                             // 加载数据的线程

        private HISPlus.OleDbAccess his = null;
        private HISPlus.OracleAccess mobile = null;
        #endregion
        ...
        public UserTrace()
        {
            InitializeComponent();
            _id = "00060";
            _guid = "4320767B-15A4-4b75-B329-60D4D36B5260";
        }
        #endregion 

        #region　窗体加载
        private void UserTrace_Load(object sender, EventArgs e)
        {
            try
            {
                tracer.WardCode = GVars.User.DeptCode;

                threadTracer = new Thread(new ThreadStart(tracer.Trace));

                threadTracer.IsBackground = true;
                //threadTracer.Priority = ThreadPriority.BelowNormal;
        ...
        #region   窗体关闭
        private void UserTrace_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
                tracer.Exit();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                txtTrace.Text += "\r\n" + ds.Tables[0].Rows[0]["DEPT_NAME"].ToString();
            }
            catch (Exception ex)
            {
