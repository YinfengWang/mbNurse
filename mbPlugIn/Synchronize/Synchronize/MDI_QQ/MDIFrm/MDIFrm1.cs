...
namespace HISPlus
{
    public partial class MDIFrm1 : FormDo
    {
        ...
        private const string DllName = "OrdersChangeTrace.dll";
        private const string FormName = "HISPlus.OrdersChangeTraceFrm";
        ...
        private void initFrmVal()
        {
            ...
            new ResourceManager("TJ.CHSIS.ResPic", Assembly.GetExecutingAssembly());
        }
        ...
        //    navBarGroup1.GroupCaptionUseImage = NavBarImage.Large;
        //}
        ...
        private void CreateMenuItem(string mnuName, string nodeId, string iconFile)
        {
            ...
            if (!menuStrip_Function.Items.Contains(topMenu))
            ...
            foreach (DataRowView drv in dv)
            {
                ...
                else //分析科室条件中，科室是否重复，如果重复将会导致程序运行错误
                {
                    if (Path.GetExtension(fileName) == ".INI")
                    {
                        ...
                        {
                            //throw new Exception("sql文件夹下" + drv["IniPath"] + "中filter病区条件为空！");
                        }
                        else
                        {
        ...
        private void Menu_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                //frmShow = Activator.CreateInstance(frmShow.GetType()) as Form;
